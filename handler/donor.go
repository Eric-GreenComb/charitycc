...
// OpenDonorAccount register donor
func OpenDonorAccount(store store.Store, args []string) ([]byte, error) {
	...
	base64Sign := args[2]
	donorSign, err := base64.StdEncoding.DecodeString(base64Sign)
	if err != nil {
	...
	bVerify := utils.RsaVerify(crypto.SHA256, hash[:], []byte(publicKey), donorSign)
	if !bVerify {
...
func QueryDonorAccount(store store.Store, args []string) ([]byte, error) {
	if len(args) != 1 {
		return nil, errors.IncorrectNumberArguments
	}
